...
*
* Task 2: Preemptive scheduling, SRTN (Shortest Remaining Time Next) with a time quantum of 3
* task2-31316115.c is a program built to simulate process scheduling based on SRTN algorithm with a time quantum of 3, a file containing processes information is read and used to calculate
...
typedef struct {
    ...
    int quantum;            // The time quantum of how long process has run
...
/*
 * Function: getShortestRemainingProcess
 * ------------------------------------------------------------------------------------------------------------------------------------------------------------------
 *  Get a process with shortest remaining time
 *
 ...
 *
 *  returns: position index in array of process with shortest remaining time
 */
int getShortestRemainingProcess(pcb_t pcb_array[], int process_in_schedule, int shortest);
...
int main (int argc, char *argv[]) {
    FILE *fptr;
    char *infilename;
    char *outfilename = "results-task2.txt";
    int num_of_process;
    ...
    while (done == 0) {
        ...
        // Select a process with shortest remaining time to run
        shortest = getShortestRemainingProcess(pcb_array,process_in_schedule,shortest);
        ...
        // Continue running a previous process
            ...
            pcb_array[shortest].quantum++;
            pcb_array[shortest].runTime++;
            pcb_array[shortest].remainingTime = pcb_array[shortest].serviceTime - pcb_array[shortest].runTime;

            // Set back to ready if quantum is reached
            if (pcb_array[shortest].quantum == max_quantum) {
                pcb_array[shortest].state = READY;
            }
        }
...
int getShortestRemainingProcess(pcb_t pcb_array[], int process_in_schedule, int shortest) {
    if (pcb_array[shortest].state != RUNNING) {
        for (int j = 0; j<process_in_schedule; j++) {
            ...
            if (pcb_array[j].state == READY) {
                // If previous shortest process is terminated/ found a process with shortest remaining time
                if (pcb_array[shortest].state == EXIT || pcb_array[j].remainingTime < pcb_array[shortest].remainingTime) {
                    shortest = j;
                }
            }
        }
